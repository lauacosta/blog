...
class HTMLRenderer {
    ...
    renderAttributes(
        ...
        if (node.pos) {
            ...
            result +=
                ` data-startpos="${sp.line}:${sp.col}:${sp.offset}" data-endpos="${ep.line}:${ep.col}:${ep.offset}"`;
        }
        return result;
    }
    ...
    inTags(
        ...
        return `${
            this.renderTag(tag, node as AstNode, extraAttrs)
        }${afterOpenSpace}${
    ...
    addBacklink(note: string, ident: number): string {
        const backlink =
            `<a href="#fnref${ident}" role="doc-backlink">\u21A9\uFE0E</a>`;
        if (/\<\/p\>[\r\n]*$/.test(note)) {
        ...
        switch (node.tag) {
            ...
            case "code_block": {
                ...
                if (node.lang) {
                    result += ` class="language-${
                        this.escapeAttribute(node.lang)
                    }"`;
                }
                result += ">";
            ...
            case "str": {
                if (node.attributes || node.autoAttributes) {
                    return `${this.renderTag("span", node)}${
                        this.escape(node.text)
                    }</span>`;
                } else {
            ...
            case "image": {
                ...
                if (node.reference) {
                    const ref = this.references[node.reference] ||
                        this.autoReferences[node.reference];
                    if (ref) {
...
export { HTMLRenderer, renderHTML };
